...
REMORA::timeStepML (Real time, int /*iteration*/)
{
    ...
    for (int lev=0; lev <= finest_level; lev++) {
        ...
        if ( (solverChoice.boundary_from_netcdf) && (lev==0) ) {
            ...
            FillPatch(lev, t_old[lev], *xvel_new[lev], xvel_new, xvel_bc(), BdyVars::u,0,true,true);
            FillPatch(lev, t_old[lev], *yvel_new[lev], yvel_new, yvel_bc(), BdyVars::v,0,true,true);
